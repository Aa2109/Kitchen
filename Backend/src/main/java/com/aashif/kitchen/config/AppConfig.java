    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
             .authorizeHttpRequests(auth-> auth
            		 ...
            		 .requestMatchers("/api/**").authenticated()//.requestMatchers("/test").permitAll()
...
private CorsConfigurationSource corsConfigurationSource() {
	...
	return new CorsConfigurationSource() {
		...
		public CorsConfiguration getCorsConfiguration(HttpServletRequest request) {
			...
			//access from front  end only these url
			cfg.setAllowedOrigins(Arrays.asList(
					"https://Aa2109/aashif'skitchen",
					"http://localhost:3000"
					));
			// methods which are allowed
			cfg.setAllowedMethods(Collections.singletonList("*"));//(CRUD operation allowed here)
